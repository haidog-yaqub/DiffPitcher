...
class UNetVC(BaseModule):
    def __init__(self,
        ...
        # scale spectrum from world vocoder
        if self.use_sp:
            print('using spectrum as content input')
            self.sp_pre = torch.nn.Sequential(torch.nn.Conv2d(1, 8, 1),
                                              torch.nn.SiLU(),
                                              torch.nn.Conv2d(8, 1, 1))

        # (nosiy_x + content_x) + (timbre + pitch)
        dim_in = 2
        dim_in += dim_cond
